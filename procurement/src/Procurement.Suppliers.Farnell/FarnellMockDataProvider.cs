...
{
    /// <summary>
    /// Fixed, realistic test data for FarnellAdapter. One exact part reproduces the worked
    /// example from spec §7 (12.000 pcs, reel 2.500 @ €0,115); anything else gets deterministic
    /// pseudo-realistic values derived from the manufacturer+MPN.
    /// </summary>
    public static class FarnellMockDataProvider
    {
        public const string ExampleManufacturer = "Vishay";
        public const string ExampleMpn = "CRCW060310K0FKEA";
        private const string ExampleSupplierPartNumber = "FN-CRCW060310K0FKEA";
        ...
        public static string BuildSupplierPartNumber(string manufacturer, string manufacturerPartNumber)
        {
            ...
            var suffix = DeterministicMock.Hash($"{manufacturer}|{manufacturerPartNumber}|farnell") % 1000000;
            return $"FN-{suffix:D6}";
        }
        ...
        public static SupplierAvailability BuildAvailability(string supplierPartNumber)
        {
            ...
            var available = DeterministicMock.NextInt(supplierPartNumber + "|avail", 500, 100000);
            var leadTime = DeterministicMock.NextInt(supplierPartNumber + "|lead", 3, 12);
            return new SupplierAvailability
            {
                SupplierPartNumber = supplierPartNumber,
        ...
        public static SupplierPricing BuildPricing(string supplierPartNumber, int quantity)
        {
            if (supplierPartNumber == ExampleSupplierPartNumber)
            {
                return new SupplierPricing
                {
                    ...
                    {
                        new PriceBreak { BreakQuantity = 2500, UnitPrice = 0.115m },
                        new PriceBreak { BreakQuantity = 10000, UnitPrice = 0.108m },
                        new PriceBreak { BreakQuantity = 25000, UnitPrice = 0.099m }
                    }
                };
            }

            var reelSize = DeterministicMock.Pick(supplierPartNumber + "|reel", new[] { 1000, 1500, 2500, 4000, 6000 });
